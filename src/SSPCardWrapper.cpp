...
SSPCardWrapper::set_running(bool should_run)
{
  ...
  TLOG_DEBUG(TLVL_WORK_STEPS) << "SSPCardWrapper Active state was toggled from " << std::boolalpha << was_running
                              << " to " << should_run;
}
...
SSPCardWrapper::configure_device(const data_t& /*args*/)
{
  ...
  std::vector<dunedaq::sspmodules::sspcardreader::RegisterValues> m_hardware_configuration =
  ...
  TLOG(TLVL_FULL_DEBUG) << "SSPCardWrapper: Hardware configuration seq has size : " << m_hardware_configuration.size()
                        << std::endl;
  ...
  for (auto regValuesIter = m_hardware_configuration.begin(); regValuesIter != m_hardware_configuration.end();
    ...
    TLOG(TLVL_FULL_DEBUG) << "SSPCardWrapper: Hardware configuration for regsiter name: " << m_name
