...
SSPLEDCalibWrapper::configure(const data_t& args)
{
  ...
  m_partition_number = m_cfg.partition_number; // this should be 0-3
  ...
  m_device_interface->ConfigureLEDCalib(
    args); // This sets up the ethernet interface and make sure that the pdts is synched
  ...
  if (m_cfg.pulse_mode == "single") {
    m_single_pulse = true;
    TLOG(TLVL_FULL_DEBUG) << "SSPLEDCalibWrapper: I think that you want SSP LED Calib module to be in single pulse..."
                          << std::endl;
  } else if (m_cfg.pulse_mode == "burst") {
    m_burst_mode = true;
    TLOG(TLVL_FULL_DEBUG) << "SSPLEDCalibWrapper: I think that you want SSP LED Calib module to be in BURST MODE..."
                          << std::endl;
  }
  ...
  // if there are "literal" entries in the configuration they are explicit writes to the specified register with given
  // value these literal entries are paresed and applied last after any other parameters so this method call needs to be
  // after the other configuration calls
...
SSPLEDCalibWrapper::start(const data_t& args)
{
  ...
  if (m_run_marker.load()) {
    TLOG_DEBUG(TLVL_ENTER_EXIT_METHODS) << "Run Marker says that SSPLEDCalibWrapper card " << m_board_id
                                        << " is already pulsing...";
    return;
  }
  ...
  for (unsigned int counter = 0; counter < m_number_channels; counter++) {
    ...
    unsigned int timing_regVal = 0x0; // the highest byte sets 8 (pdts trigger) + 1 (953.5 Hz) for burst mode, but 8
                                      // (pdts trigger) + 7 (single shot) for single
    if (m_single_pulse) {
    ...
    TLOG(TLVL_FULL_DEBUG) << "Channel map is 0x" << std::hex << m_channel_mask << " and the comparison is 0x"
                          << (1 << counter) << std::endl;
    if ((m_channel_mask & ((unsigned int)1 << counter)) == ((unsigned int)1 << counter)) {
      ...
      TLOG(TLVL_FULL_DEBUG) << "Will turn on " << std::dec << counter << " channel at bias register 0x" << std::hex
                            << bias_regAddress << " with bias value 0x" << bias_regVal << std::endl;
      TLOG(TLVL_FULL_DEBUG) << " and set the width regsiter 0x" << std::hex << timing_regAddress << " to value of 0x"
                            << timing_regVal << std::dec << std::endl;
      ...
      if ((counter == 7) && !((m_channel_mask & ((unsigned int)1 << counter)) == ((unsigned int)1 << (counter - 1)))) {
        // this is a really convoluted situation where for the very specific 12 channel board that arrived at CERN in
        // June 2022 the bias for channel 7 was not working and so the bias is taken from channel 6 which means that if
        // channel 7 is to be turned on while channel 6 is masked off you still have to bias channel 6 in order for the
        // led on channel 7 to emmit light
      ...
      TLOG(TLVL_FULL_DEBUG) << "Will turn off channel " << std::dec << counter << " at timing register 0x" << std::hex
                            << timing_regAddress << std::dec << std::endl;
      m_device_interface->SetRegister(bias_regAddress, bias_regVal);     // BIAS_DAC_CONFIG_N
      m_device_interface->SetRegister(timing_regAddress, timing_regVal); // cal_CONFIG_N
    }
  }
...
SSPLEDCalibWrapper::stop(const data_t& /*args*/)
{
  ...
  if (!m_run_marker.load()) {
    TLOG_DEBUG(TLVL_ENTER_EXIT_METHODS) << "The run_marker says that SSPLEDCalibWrapper card " << m_board_id
                                        << " is already stopped, but stopping anyways...";
  }
...
SSPLEDCalibWrapper::manual_configure_device(const data_t& /*args*/)
{
  ...
  TLOG(TLVL_FULL_DEBUG) << "SSPLEDCalibWrapper: Hardware configuration seq has size : "
                        << m_hardware_configuration.size() << std::endl;
...
SSPLEDCalibWrapper::validate_config(const data_t& args)
{
  ...
  if (!((m_cfg.number_channels == 5) || (m_cfg.number_channels == 12))) {
    std::stringstream ss;
    ss << "ERROR: Incorrect number_channels value " << m_cfg.number_channels << " is not equal to 5 or 12!!!"
       << std::endl;
    TLOG() << ss.str();
    throw ConfigurationError(ERS_HERE, ss.str());
  }
  ...
  if (m_cfg.channel_mask > 4095) {
    std::stringstream ss;
    ss << "ERROR: Incorrect channel_maks value " << m_cfg.channel_mask << " is higher than the limit of 4095!!!"
       << std::endl;
    TLOG() << ss.str();
    throw ConfigurationError(ERS_HERE, ss.str());
  }
  ...
  if (!((m_cfg.pulse_mode == "single") || (m_cfg.pulse_mode == "burst"))) {
    std::stringstream ss;
    ss << "ERROR: Incorrect pulse_mode value is " << m_cfg.pulse_mode << ", it must be single, or burst." << std::endl;
    TLOG() << ss.str();
    throw ConfigurationError(ERS_HERE, ss.str());
  }
  ...
  if (m_cfg.double_pulse_delay_ticks > 4095) {
    std::stringstream ss;
    ss << "ERROR: Strange!! double_pulse_delay_ticks value is " << m_cfg.double_pulse_delay_ticks
       << ", which is greater than the limit of 4095" << std::endl;
    TLOG() << ss.str();
    // throw ConfigurationError(ERS_HERE, ss.str());
  }
  ...
  if (m_cfg.burst_count > 10000) {
    std::stringstream ss;
    ss << "ERROR: Strange!! burst_count value is " << m_cfg.burst_count
       << ", which is more time than in a drift readout window" << std::endl;
    TLOG() << ss.str();
    // throw ConfigurationError(ERS_HERE, ss.str());
  }
  ...
  if (m_cfg.pulse1_width_ticks > 255) {
    std::stringstream ss;
    ss << "ERROR: Incorrect pulse1_width_ticks value is " << m_cfg.pulse1_width_ticks
       << ", which is greater than the limit of 255!!!" << std::endl;
    TLOG() << ss.str();
    throw ConfigurationError(ERS_HERE, ss.str());
  }
  ...
  if (m_cfg.pulse2_width_ticks > 255) {
    std::stringstream ss;
    ss << "ERROR: Incorrect pulse2_width_ticks value is " << m_cfg.pulse2_width_ticks
       << ", which is greater than the limit of 255!!!" << std::endl;
    TLOG() << ss.str();
    throw ConfigurationError(ERS_HERE, ss.str());
  }
  ...
  if (m_cfg.pulse_bias_percent_270nm > 100) {
    std::stringstream ss;
    ss << "ERROR: Incorrect pulse_bias_percent_270nm value is " << m_cfg.pulse_bias_percent_270nm
       << ", which is greater than 100 percent!!!" << std::endl;
    TLOG() << ss.str();
    throw ConfigurationError(ERS_HERE, ss.str());
  }
  ...
  if (m_cfg.pulse_bias_percent_367nm > 100) {
    std::stringstream ss;
    ss << "ERROR: Incorrect pulse_bias_percent_367nm value is " << m_cfg.pulse_bias_percent_367nm
       << ", which is greater than 100 percent!!!" << std::endl;
    TLOG() << ss.str();
    throw ConfigurationError(ERS_HERE, ss.str());
  }
...
//                     "channel_mask": 4095, //12 chan binary bit mask, has to be given in decimal, e.g. 128 turns on
//                     chan 7 (count from 0) "pulse_mode": "burst","single" "double_pulse_delay_ticks": 100, //number of
//                     ticks between pulses when in double mode < 4096 "burst_count": 1000, //count of pulses to give in
//                     burst mode, must be < 10000 "pulse1_width_ticks": 100, //one tick is ~4 ns, must be less than 256
//                     "pulse2_width_ticks": 100, //one tick is ~4 ns, must be less than 256
//                     "pulse_bias_percent_270nm": 100, //this is percentage of the bias that is supplied to the SSP
//                     card "pulse_bias_percent_367nm": 100,  //note that the bias might be anything 0V - 35V
// "hardware_configuration": [ //included to be able to overwrite default config values
//     {
//         "regname": "Literal",
//         "hexvalues": [2147484776,2147483648] //this is 0x80000468 with value 0x80000000 which is pdts_cmd_control_2
//         and turns on the front panel LEDs
//     }
// ]
//                 },
//                 "match": "ssp_led_calib"
