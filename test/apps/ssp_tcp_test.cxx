...
int
sendReceive(int sockfd, Ctrl_Packet* tx, Ctrl_Packet* rx)
{
  for (int attempt = 0; attempt < RETRY_COUNT; ++attempt) {
    if (send(sockfd, (const char*)tx, tx->header.total_pkt_len, 0) < 0) {
      perror("Send failed");
      continue;
    }

    if (recv(sockfd, (char*)rx, sizeof(Ctrl_Packet), 0) < 0) {
...
int
DeviceRead(int sockfd, unsigned int address, unsigned int* value)
{
  Ctrl_Packet tx, rx;
...
main(int argc, char* argv[])
{
  ...
  int sockfd = connectToDevice(device_ip);
  if (sockfd < 0) {
    return EXIT_FAILURE;
  }

  if (DeviceRead(sockfd, address, &value) == 0) {
